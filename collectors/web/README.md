...
각 Reel은 먼저 현재 페이지의 embedded JSON과 화면에 완전한 정수로 표시된 DOM을 분석합니다.
필수 정확값이 빠진 경우에는 현재 페이지가 이미 보낸 JSON Response를 최대 20초 동안 수동 관찰해 부족한 필드를 보완합니다. 그 뒤에도 조회수가 없으면 작성자 프로필의 Reels 탭으로 이동해 최대 30회 스크롤하며 같은 Reel 카드를 찾습니다. 카드 조회수가 `6591`처럼 완전한 정수면 DOM 값을 바로 사용하고, `1.5만`처럼 축약 표기일 때는 카드를 클릭해 Instagram 화면이 보내는 원본 `play_count` Response를 최대 1분 더 기다립니다.
이 fallback은 일반 브라우저 이동과 클릭만 사용하며 별도 media-info API를 직접 호출하지 않습니다. 그 응답에도 정확 정수가 없으면
...
최초 수집 후보에는 업로드 후
30일 이내 필터를 적용합니다. 각 Reel은 최초 수집 후 실행 시간 내에서 `+4시간` 간격으로 반복
재수집합니다. 나머지 시간에는 신규 탐색 없이 기한이 된
...
| 창당 신규 저장 | 최대 300개 |
| 최초 수집 업로드 범위 | 최근 30일 |
| 같은 Reel 재수집 | 최초 수집 시각부터 4시간마다, 실행 종료 전까지 |
