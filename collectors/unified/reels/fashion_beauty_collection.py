...
def _generic_collector_options(
    *,
    ...
    if config.collector_mode == "web":
        arguments.append("--no-android-metrics")
    elif config.collect_hashtag_media_count:
...
def active_keyword_window_index(
    config: RunConfig,
    started_at: datetime,
    now: datetime,
) -> int:
    """Return the keyword group for the active discovery window.

    A single-domain ``--new-only`` run is intentionally pinned to its first
    hashtag group.  It may scan that same group again in a later 30-minute
    window, but it must not reset into the next group.  Fashion+Beauty keeps
    its existing alternating-domain rotation.
    """
    if config.new_only and len(config.domains) == 1:
        return 1
    ...
    *,
    ...
    try:
        async with SupervisorLock(lock_path):
            if config.collector_mode == "hybrid":
                android_log_relays = [
            ...
            while not stop_event.is_set() and clock() < ends_at:
                ...
                if eligible_jobs:
                    ...
                    if (
                        ...
                        if result:
                            print(
                                f"[PYTHON] {selected.name}: collector invocation failed: {error}",
                                file=sys.stderr,
                            )
