...
if __package__:
    from .android_metric_queue import (
        REEL_HISTORY_DIRECTORY,
        _android_metric_queue_paths,
        android_metric_queue_counts,
        android_metric_queue_root,
        clear_collection_stop_request,
        collection_stop_path,
    )
    ...
    from .reel_records import (
        ...
        calculate_reel_derived_fields,
        caption_without_hashtags,
        enrich_reel_collection_rows,
        exact_follower_result_from_metadata,
        exact_nonnegative_integer,
        exact_nonnegative_number,
        exact_profile_post_count,
        exact_visible_profile_follower_count,
        exact_visible_profile_following_count,
        exact_visible_profile_post_count,
        extract_hashtags,
        filter_new_urls,
        first_follower_count,
        follower_count_from_instagram_data,
        follower_count_success,
        format_audio_name,
        has_ad_signal,
        has_exact_engagement_metadata,
        hashtag_page_url,
        is_instagram_hashtag_surface,
        is_instagram_reels_surface,
        is_like_count_marked_unavailable,
        is_search_grid_card_visible,
        isoformat_kst,
        isoformat_utc,
        js_round,
        normalize_reel_url,
        normalize_search_grid_reel_url,
        normalize_upload_time,
        parse_datetime,
        parse_follower_count,
        parse_hashtag_query,
        parse_metric_count,
        parse_snapshot_field,
        reel_detail_page_url,
        media_is_advertisement,
        profile_category_from_data,
        profile_category_from_visible_text,
        profile_snapshot_from_instagram_data,
        resolve_follower_result,
        shortcode_to_media_id,
        snapshot_labels,
        truncate_caption,
        utc_now,
        visible_search_grid_reel_urls,
    )
    from .reel_store import replace_file_with_retry, write_csv_records, write_json_records
else:
    ...
    from reels import collection_pause
    from reels.android_metric_queue import (  # type: ignore[no-redef]
        REEL_HISTORY_DIRECTORY,
        _android_metric_queue_paths,
        android_metric_queue_counts,
        android_metric_queue_root,
    ...
    from reels.browser_runtime import load_playwright, locate_browser_executable, safe_close  # type: ignore[no-redef]
    from reels.collection_diagnostics import CollectorDiagnostics, sanitize_log_value  # type: ignore[no-redef]
    from reels.reel_records import (  # type: ignore[no-redef]
        _hashtags,
        _dict,
        CSV_FIELDS,
        REEL_CHANGE_FIELDS,
    ...
    from reels.reel_store import replace_file_with_retry, write_csv_records, write_json_records  # type: ignore[no-redef]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--profile-dir", type=Path, default=PYTHON_VERSION_ROOT / "browser_profile" / ".instagram_chrome_profile")
...
class CrawlerAccessError(RuntimeError):
    def __init__(
        ...
        *,
        ...
        terminate_after_resume: bool = False,
    ) -> None:
        super().__init__(message)
        self.code = code
        self.refresh_attempted = refresh_attempted
        self.retry_after_seconds = retry_after_seconds
        self.terminate_after_resume = terminate_after_resume
...
class InstagramRateLimitState:
    ...
    def observe(self, response: Any) -> None:
        ...
        # consumed, a second 429 is a terminal stop for this run: do not put
        # the process back into an endless pause/resume loop.
        resumed = self.resume_count > 0 or (self.pause_seen and not collection_pause.is_paused())
        if resumed and self.resume_count == 0:
            # A Playwright target without a discoverable Page owner can still
            # pass through the pause wrapper.  Infer that its marker was
            # cleared by the operator so the next 429 remains terminal.
            self.resume_count = 1
        self.terminate_after_resume = resumed
        if self.manual_pause and not self.terminate_after_resume:
    ...
    def raise_if_limited(self) -> None:
        ...
        raise CrawlerAccessError(
            ...
            terminate_after_resume=self.terminate_after_resume,
        )

    async def wait_if_limited(self) -> None:
        if self.manual_pause:
            if self.terminate_after_resume:
    ...
    def mark_resumed(self) -> None:
        """Record the operator's first resume and clear the active latch."""
        if self.terminate_after_resume or not self.limited:
            return
        self.resume_count += 1
        self.clear_limit()
    ...
    *,
    ...
    try:
        ...
        async def save_rate_limited_reel(url: str, error: str) -> bool:
            ...
            nonlocal captured
            # A repeated 429 ends the run.  Do not turn the failed visit into
            # a new partial snapshot; the rows saved before the terminal
            # response remain the complete refreshable checkpoint.
            if (
                reel_store is None
                or not hybrid_android_metrics
                or (rate_limit_state is not None and rate_limit_state.terminate_after_resume)
            ):
                return False
            normalized = normalize_reel_url(url)
            if normalized is None:
        ...
        if isinstance(error, CrawlerAccessError) and error.code == "rate_limited":
            ...
            if (
                rate_limited_reel_saver is not None
                and normalize_reel_url(fallback_url)
                and not error.terminate_after_resume
            ):
                try:
            ...
            if error.terminate_after_resume:
                stop_event.set()
                print(
                    "[PYTHON] Instagram HTTP 429 occurred again after resume; "
                    "stopping this collection. Previously saved rows are available "
                    "for refresh."
                    + retry_hint,
                    file=sys.stderr,
                )
            ...
            try:
                await asyncio.to_thread(
                    ...
                    terminated_after_resume=error.terminate_after_resume,
                )
            except Exception:
                pass
        if android_pipeline is not None:
        ...
        if status_reporter:
            try:
                await status_reporter.finish("failed", {
                    ...
                    "terminated_after_resume": bool(getattr(error, "terminate_after_resume", False)),
                    "refresh_only": bool(getattr(error, "terminate_after_resume", False)),
                })
            except Exception:
                pass
        raise
    ...
    while True:
        try:
            ...
            if error.terminate_after_resume:
                ...
                print(
                    "[PYTHON] HTTP 429 재발( resume 후 ): 수집을 종료합니다. "
                    "이전까지 저장된 행은 refresh로 재수집할 수 있습니다.",
                    file=sys.stderr,
                )
                return 2
    ...
    *,
    ...
    try:
        ...
        for index, options in enumerate(options_list):
            result = await run_one(index, options)
            results.append(result)
            state = getattr(context, "_instagram_collector_rate_limit_state", None)
            if state is None:
                state = _CONTEXT_RATE_LIMIT_STATES.get(id(context))
            if getattr(state, "terminate_after_resume", False):
                if external_stop_event is not None:
                    external_stop_event.set()
                # Preserve the one-result-per-dataset contract for scheduler
                # callers while preventing any later dataset from issuing
                # requests after the terminal repeated 429.
                results.extend([2] * (len(options_list) - len(results)))
                break
        return results
    finally:
        await safe_close(context)
