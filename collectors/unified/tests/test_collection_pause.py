...
from reels import collection_pause as pause
...
class ManualPauseTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_guard_stops_after_second_429_following_resume(self):
        from reels.instagram_reels_browser import InstagramRateLimitState

        calls = []
        state = InstagramRateLimitState(manual_pause=True)

        class Page:
            _instagram_collector_rate_limit_state = state

            async def goto(self):
                calls.append("goto")
                response = SimpleNamespace(
                    url=f"https://www.instagram.com/reel/{len(calls)}/",
                    status=429,
                    headers={},
                    page=self,
                )
        ...
        with patch.object(pause, "pause") as pause_call, patch.object(
            pause, "wait", new=wait_without_terminal
        ):
            guarded_goto = pause.guarded(Page.goto)
            with self.assertRaises(Exception) as raised:
                await guarded_goto(Page())

        self.assertEqual(calls, ["goto", "goto"])
        pause_call.assert_called()
        self.assertFalse(pause.is_paused())
        self.assertTrue(getattr(raised.exception, "terminate_after_resume", False))

    async def test_android_checks_pause_before_adb_command(self):
        from reels.android_reel_metrics import AdbAndroidUiDriver
        ...
        with patch.object(pause, "wait_sync", new=wait_sync), patch("reels.android_reel_metrics.subprocess.run", new=run):
