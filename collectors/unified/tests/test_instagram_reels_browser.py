...
from reels import instagram_reels_browser as reels_browser
from reels.instagram_follower_enricher import (
    FollowerEnricher,
    read_csv_objects,
    user_history_path,
)
...
from reels.instagram_reels_browser import (
    AndroidMetricPipeline,
    CSV_FIELDS,
    CollectorLock,
    LongReelStore,
    PYTHON_VERSION_ROOT,
    ...
    build_hashtag_grid_candidate_record,
    ...
    missing_python_to_android_handoff_fields,
    ...
    python_to_android_handoff_delay_seconds,
...
class CollectorUtilityTests(unittest.TestCase):
    ...
    def test_collection_logs_are_written_to_separate_source_files(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            python_log = reels_browser.append_collection_log(
                directory,
                "python",
                "reel_saved",
                url="https://www.instagram.com/reels/python/",
            )
            android_log = reels_browser.append_collection_log(
                directory,
                "android",
            ...
            self.assertEqual(python_log.name, "python.log")
            self.assertEqual(android_log.name, "android.log")
    ...
    def test_cli_defaults_and_long_run_controls(self) -> None:
        ...
        self.assertEqual(defaults.profile_dir, PYTHON_VERSION_ROOT / "browser_profile" / ".instagram_chrome_profile")
        options = parse_args([
    ...
    def test_python_to_android_handoff_does_not_wait_for_android_readable_counts(self) -> None:
        ...
        record.update({
            ...
            "share_count": "",
            "repost_count": "",
            # User snapshots are owned by Python users.xlsx and must not
            # make an otherwise complete Reel wait for Android.
            "follower_count": "",
        })
    ...
    def test_python_to_android_handoff_waits_three_seconds_when_web_data_is_missing(self) -> None:
        ...
        self.assertEqual(missing_python_to_android_handoff_fields(record), ("comment_count",))
        self.assertEqual(
    ...
    def test_hashtag_grid_candidate_uses_the_card_shortcode_without_detail_navigation(self) -> None:
        candidate = build_hashtag_grid_candidate_record(
            "https://www.instagram.com/p/DbvAYbzp-87/?img_index=1",
            {
                "userId": "123",
                "username": "creator",
                "caption": "grid caption #style",
                "uploadedAt": "2026-08-30T12:00:00.000Z",
                "videoDurationSeconds": 14.5,
            },
        )

        self.assertEqual(candidate["url"], "https://www.instagram.com/reels/DbvAYbzp-87/")
        self.assertEqual(candidate["shortcode"], "DbvAYbzp-87")
        self.assertEqual(candidate["username"], "creator")
        self.assertEqual(candidate["uploadedAt"], "2026-08-30T12:00:00.000Z")
    ...
    def test_hashtag_or_and_metric_parsing(self) -> None:
        ...
        self.assertEqual(
            ...
            "https://www.instagram.com/explore/tags/%ED%8C%A8%EC%85%98%EC%8A%A4%ED%83%80%EA%B7%B8%EB%9E%A8/",
        )
    ...
    def test_visible_html5_video_duration_is_saved_when_page_metadata_omits_it(self) -> None:
        collected = build_collected_record(
            {
                "url": "https://www.instagram.com/reels/DA55rVCNIrp/",
                "username": "example",
                "uploadedAt": "2026-08-24T00:00:00.000Z",
                "videoDurationSeconds": 17.4,
            },
            {"userId": "123", "username": "example", "uploadedAt": "2026-08-24T00:00:00.000Z"},
            "2026-08-24T01:00:00.000Z",
        )

        self.assertEqual(collected["video_duration_seconds"], 17.4)
...
class CollectorAsyncTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_hybrid_page_metadata_skips_metric_wait_and_creator_profile_fallback(self) -> None:
        class Locator:
            async def all_text_contents(self) -> list[str]:
                return []
        ...
        async def wait_for_any_response(
            _shortcode: str,
            _metadata: dict[str, dict[str, object]],
            timeout_milliseconds: int,
        ) -> dict[str, object]:
    ...
    async def test_direct_reel_info_parses_json_in_the_browser_and_returns_only_metric_fields(self) -> None:
        ...
        self.assertEqual(page.media_id, "64")
        self.assertIn("response.json()", page.script)
        self.assertNotIn("response.text()", page.script)
        self.assertEqual(metadata["viewCount"], 24_585)
        self.assertEqual(metadata["repostCount"], 6)
    ...
    async def test_persistent_context_keeps_its_first_startup_tab(self) -> None:
        class Page:
            def __init__(self) -> None:
                self.close_calls = 0
        ...
        first, extra = Page(), Page()

        class Context:
            pages = [first, extra]

        class Chromium:
            async def launch_persistent_context(self, *_args: object, **_kwargs: object) -> Context:
                return Context()

        with tempfile.TemporaryDirectory() as directory:
            options = parse_args(["--profile-dir", str(Path(directory) / "profile")])
            _browser, context = await launch_collection_context(Chromium(), "browser.exe", options)
    ...
    async def test_collection_startup_reuses_persistent_context_initial_tab(self) -> None:
        class Page:
            url = "about:blank"
        ...
        class Context:
            ...
            async def new_page(self) -> Page:
                raise AssertionError("The initial persistent-context tab should be reused.")
    ...
    async def test_long_store_exports_recollections_as_rows(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            expected_fields = [
                ...
                "comment_count", "share_count", "repost_count", "saved_count", "follower_count",
                "reaction_rate",
    ...
    async def test_android_metrics_enrich_the_browser_snapshot_without_a_second_row(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            store = await LongReelStore.create(Path(directory) / "reels_rows.csv", 100, "rows")
            browser_record = reel_record(1, "2026-01-01T00:00:00.000Z")
            browser_record.update({"view_count": "", "like_count": "", "comment_count": "", "audio_name": ""})

            await store.append(browser_record)
            enriched_record = {
                **browser_record,
                "view_count": 624_267,
                "like_count": 15_691,
            ...
            self.assertTrue(await store.enrich_latest_snapshot(enriched_record))

            self.assertEqual(len(store.rows), 1)
            self.assertEqual(store.rows[0]["view_count"], 624_267)
            self.assertEqual(store.rows[0]["share_count"], 11_300)
    ...
    async def test_android_metric_pipeline_queues_the_next_reel_without_waiting(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            await asyncio.to_thread(lookup_started.wait, 1)
    ...
    async def test_durable_android_queue_merges_after_python_has_moved_on(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            job_id = reels_browser.enqueue_android_metric_job(data_dir, first)
            paths = reels_browser._android_metric_queue_paths(data_dir)
            pending = paths["pending"] / f"{job_id}.json"
    ...
    async def test_idle_hashtag_jobs_yield_to_reel_metric_jobs(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            data_dir = Path(directory)
            idle_jobs = reels_browser.enqueue_android_idle_hashtag_post_count_jobs(
                data_dir,
                ["fashion", "beauty"],
            )
            metric = reels_browser.enqueue_android_metric_job(data_dir, reel_record(1))
            paths = reels_browser._android_metric_queue_paths(data_dir)

            claimed_result = reels_browser._claim_android_metric_job(paths)

            self.assertIsNotNone(claimed_result)
            assert claimed_result is not None
            _working, claimed = claimed_result
            self.assertEqual(claimed["job_id"], metric)
            self.assertEqual(len(idle_jobs), 2)
            self.assertEqual(len(list(paths["pending"].glob("idle-hashtag-*.json"))), 2)

    async def test_idle_hashtag_job_collects_the_exact_search_tag_total(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            data_dir = Path(directory)
            ...
            class TagEnricher:
                def collect_hashtag_post_counts(self, hashtags: list[str]) -> list[dict[str, object]]:
                    observed.append(hashtags)
                    return [{
                        "collected_at": "2026-08-31T00:00:00.000Z",
                        "query_hashtag": "fashion",
                        "hashtag": "fashion",
                        "post_count": 1_234,
                        "raw_post_count": "1234 posts",
                        "source": "android_search_tags_exact",
                        "status": "collected",
                        "error": "",
                    }]

            with patch.object(reels_browser, "AndroidReelMetricsEnricher", return_value=TagEnricher()):
                exit_code = await asyncio.to_thread(
                    reels_browser.run_android_metric_worker,
                    data_dir,
                    idle_seconds=0.01,
            ...
            self.assertEqual(exit_code, 0)
            self.assertEqual(observed, [["fashion"]])
